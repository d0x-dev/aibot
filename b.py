...
@app.route('/gate=braintree/key=<key>/cc=<cc_details>')
def check_cc(key, cc_details):
    if key != API_KEY:
        return jsonify({"error": "Invalid API key"}), 403
    
    if not re.match(r'^\d{15,16}\|\d{2}\|\d{4}\|\d{3,4}$', cc_details):
    ...
    try:
        # Process the CC
        result = create_payment_method(cc_details)
        response = charge_resp(result)
        elapsed_time = round(time.time() - start_time, 2)
        
        status = "Approved ✅" if "Approved" in response else "Declined ❌"
        
        # Print to terminal with proper emoji display
        terminal_response = response.replace('✅', '[APPROVED]').replace('❌', '[DECLINED]').replace('🚫', '[ERROR]')
        terminal_status = status.replace('✅', '[APPROVED]').replace('❌', '[DECLINED]')
        print(f"cc={cc_details}, status={terminal_status}, response={terminal_response}")
        
        # Return JSON with original emojis
        return jsonify({
            "cc": cc_details,
            "status": status,
            "response": response,
        ...
        logger.error(f"Error in check_cc: {str(e)}")
        error_msg = f"Error 🚫 - {str(e)}"
        print(f"cc={cc_details}, status=Error, response={error_msg}")
        return jsonify({
            "cc": cc_details,
            "status": "Error 🚫",
            "response": str(e),
